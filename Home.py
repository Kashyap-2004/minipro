...
import string
import pandas as pd
from fake_news_pipeline import predict_news, load_model_and_vectorizer, predict_image_gemini
...
# Load model and vectorizer (optional, for backward compatibility)
...
def home():
    ...
    if request.method == "POST":
        ...
        if "message" in request.form and request.form["message"]:
            ...
            print(f"[REQUEST] Predicting for news: {news_text[:100]}...")
            try:
                result, confidence = predict_news(news_text)
                print(f"[REQUEST] Prediction result: {result}, Confidence: {confidence}")
            except Exception as e:
                print(f"[REQUEST] Error during prediction: {e}")
                import traceback
                traceback.print_exc()
        ...
        # Handle image upload
            ...
            print(f"[REQUEST] Image file received: {file.filename if file else 'None'}")
            if file and file.filename and allowed_file(file.filename):
                try:
                    ...
                    print(f"[REQUEST] Analyzing image: {filename}")
                    result, confidence = predict_image_gemini(filepath)
                    print(f"[REQUEST] Image prediction result: {result}, Confidence: {confidence}")
                    
                    news_text = f"[Image Analysis] {filename}"
                    ...
                    print(f"[REQUEST] Error during image prediction: {e}")
                    import traceback
                    traceback.print_exc()
                    result = "Unable to determine"
                    confidence = 0
